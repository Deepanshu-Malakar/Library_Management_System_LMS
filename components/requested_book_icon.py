from customtkinter import *
from tkinter import *
from PIL import Image
from backend import request_books_logic
from tkinter import messagebox

class color:
    def __init__(self):
        self.base_color = "#4153A3"
        self.disable_buttons = "#535769"
        self.fine = "#DF3939"
        self.sidebar = "#292B34"
        self.book_base = "#ffffff"
        self.new_button_color = "#1A2032"
        self.new_button_color_hover = "#090B12"
        self.book_hover = "#1A2032"
        self.old_hover = "#C0BEC2"


colors = color()
set_appearance_mode("light")


class Book_icon:
    def __init__(self, master: CTkFrame, logo: str, book_name: str, author: str, edition: int, total_requests):
        self.master = master
        self.frame = CTkFrame(master=master,
                              width=360,
                              height=280,
                              fg_color=colors.book_base,
                              corner_radius=5,
                              border_color="#DAD2DF",
                              border_width=1)
        # self.reserver_id = reserver_id
        self.title = book_name
        self.author = author
        self.logo = logo
        self.logo_img = CTkImage(Image.open(logo), size=(180, 280))
        self.edition = edition
        self.copies = total_requests
        self.create_book()
        self.description()

    def create_book(self):
        self.logo_label = CTkLabel(self.frame,
                                   text="",
                                   image=self.logo_img,
                                   font=("roboto", 12),
                                   wraplength=160,
                                   justify="center")
        self.logo_label.place(x=0, y=0)

        title = self.title if len(self.title) <= 22 else self.title[:20] + ".."
        self.title_label = CTkLabel(self.frame, text=title, font=("roboto", 12, "bold"))
        self.title_label.place(x=200, y=20)

        self.author_label = CTkLabel(self.frame, text=f"By : {self.author}", font=("roboto", 12))
        self.author_label.place(x=200, y=60)

        self.edition_label = CTkLabel(self.frame, text=f"Edition : {self.edition}", font=("roboto", 12))
        self.edition_label.place(x=200, y=100)

        # Copies to Add + Entry beside it
        self.copies_label = CTkLabel(self.frame, text=f"Copies to Add :", font=("roboto", 12))
        self.copies_label.place(x=200, y=140)

        self.copies_entry = CTkEntry(self.frame, width=60, height=25, font=("roboto", 12))
        self.copies_entry.place(x=285, y=140)

        # Total Requests (showing available copies)
        self.total_requests_label = CTkLabel(self.frame,
                                             text=f"Total Requests : {self.copies}",
                                             font=("roboto", 12))
        self.total_requests_label.place(x=200, y=180)

        # Add Button
        self.add_btn = CTkButton(self.frame,
                                 text="                Add Book",
                                 font=("roboto", 12, "bold"),
                                 width=180,
                                 height=40,
                                 corner_radius=0,
                                 fg_color=colors.new_button_color,
                                 hover_color=colors.new_button_color_hover,
                                 anchor="w",
                                 command=self.add_btn_click)

        self.add_btn_img = CTkImage(Image.open("resources/icons/add book light.png"), size=(17, 17))
        self.add_btn_img_label = CTkLabel(self.frame, image=self.add_btn_img, text="",
                                          fg_color=colors.new_button_color, bg_color=colors.new_button_color)
        self.add_btn_img_label.place(x=200, y=225)
        self.add_btn.place(x=180, y=220)

        # Hover Effects
        self.add_btn.bind("<Enter>", self.btn_hover)
        self.add_btn.bind("<Leave>", self.btn_unhover)

        self.frame.bind("<Enter>", self.frame_hover)
        self.frame.bind("<Leave>", self.frame_unhover)
        self.title_label.bind("<Enter>", self.frame_hover)
        self.author_label.bind("<Enter>", self.frame_hover)
        self.copies_label.bind("<Enter>", self.frame_hover)
        self.edition_label.bind("<Enter>", self.frame_hover)
        self.logo_label.bind("<Enter>", self.frame_hover)
        self.logo_label.bind("<Leave>", self.frame_unhover)
        self.add_btn.bind("<Enter>", self.frame_hover)
        self.add_btn.bind("<Leave>", self.frame_unhover)

        # Description hover
        self.logo_label.bind("<Enter>", self.show_description)
        self.logo_label.bind("<Leave>", self.hide_description)

    # Hover Effects
    def btn_hover(self, event):
        self.add_btn_img_label.configure(fg_color=colors.new_button_color_hover, bg_color=colors.new_button_color_hover)

    def btn_unhover(self, event):
        self.add_btn_img_label.configure(fg_color=colors.new_button_color, bg_color=colors.new_button_color)

    def frame_hover(self, event):
        self.frame.configure(border_color=colors.base_color, border_width=1.5)

    def frame_unhover(self, event):
        self.frame.configure(border_color="#DAD2DF", border_width=1)

    # Click Function
    def add_btn_click(self):
        if self.copies_entry.get() == "" or not self.copies_entry.get().isdecimal():
            messagebox.showerror("Please give valid copies")
            return
        request_books_logic.add_book(self.title,self.author,self.edition,int(self.copies_entry.get()))
        self.frame.destroy()
        ...
        # backend logic placeholder

    # Description Functions
    def description(self, text="No Description"):
        self.summary = self.title + "\n\n" + text

    def show_description(self, event):
        self.logo_label.configure(text=self.summary,
                                  image=CTkImage(Image.open("resources/Books/empty book4.jpg"), size=(180, 280)))

    def hide_description(self, event):
        self.logo_label.configure(text="", image=self.logo_img)

    # Placement
    def pack(self, padx=0, pady=0):
        self.frame.pack(padx=padx, pady=pady)

    def place(self, x, y):
        self.frame.place(x=x, y=y)

    def grid(self, row, column, padx=0, pady=0):
        self.frame.grid(row=row, column=column, padx=padx, pady=pady)


if __name__ == "__main__":
    root = CTk(fg_color="#ffffff")
    b1 = Book_icon(root,
                   logo="resources/Books/harry potter1.jpg",
                   book_name="Harry Potter and the Philosopher's Stone",
                   author="J.K. Rowlings",
                   edition=1,
                   total_requests=12)
    b1.pack(padx=10, pady=10)
    root.mainloop()
